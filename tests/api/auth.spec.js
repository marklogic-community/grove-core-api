...
describe('/api/auth/status', () => {
  it('only allows GET', function() {
    return frisby.post(groveBaseUrl + 'api/auth/status').expect('status', 405);
  });
...
describe('/api/auth/login', () => {
  it('only allows POST', function() {
    return frisby.get(groveBaseUrl + 'api/auth/login').expect('status', 405);
  });
  ...
  it('should expect username and password', function() {
    return frisby.post(groveBaseUrl + 'api/auth/login').expect('status', 400);
  });
...
describe('/api/auth/logout', () => {
  it('only allows POST', function() {
    return frisby.get(groveBaseUrl + 'api/auth/logout').expect('status', 405);
  });
  ...
  it("doesn't fail when not logged in", function() {
    return frisby.post(groveBaseUrl + 'api/auth/logout').expect('status', 204);
  });
...
describe('/api/auth/profile', () => {
  it('only allows GET or POST', function() {
    return frisby.put(groveBaseUrl + 'api/auth/profile').expect('status', 405);
  });
  ...
  it('GET requires logging in', function() {
    return frisby.get(groveBaseUrl + 'api/auth/profile').expect('status', 401);
  });
